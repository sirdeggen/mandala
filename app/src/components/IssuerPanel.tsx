...
export default function IssuerPanel({ assetId: controlledAssetId }: IssuerPanelProps = {}) {
  ...
  const exceedsReserve = fullReserve && Number.isFinite(issueAmountNum) && issueAmountNum > availableReserve
  ...
  const handleIssue = () => {
    ...
    if (fullReserve && Number(issueAmount) > availableReserve) {
      toast.error(`Full-reserve enforcement: issuance can't exceed the available reserve balance (${availableReserve.toLocaleString('en-US')}).`)
      return
    }
